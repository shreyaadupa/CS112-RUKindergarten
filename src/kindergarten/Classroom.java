...
public class Classroom {
    ...
    public void enterClassroom ( String filename ) {
        ...
        //create object of the type Studednt with the student info
        //insert the student object at front of the linkedlist called studentsInLine

        //loop through the number of students 
        //make each a student object with its respective info
        //add that student object to the head of the linked list
        ...
        for(int i = 0; i<numStudents; i++)
        {
            ...
            Student newStudent = new Student(first, last, height);
            SNode addingStudent = new SNode(newStudent,studentsInLine);
     ...
     public void seatStudents () {
        ...
        // WRITE YOUR CODE HERE
                ...
                for(int i = 0; i<studentsSitting.length; i++)
                {
                    ...
                    {
                        if(studentsInLine!=null && seatingLocation[i][j]==true && studentsSitting[i][j]==null)
                        {
    ...
    public void insertMusicalChairs () { 
        ...
        for(int r = 0; r<studentsSitting.length; r++)
        {
            for(int c = 0; c<studentsSitting[r].length; c++)
            {
                ...
                {
                    if(musicalChairs == null)//if null/empty
                    {
     ...
     */
    public void moveStudentFromChairsToLine(int size) {//FIX THIS ONE AND I THINK REST OF IT SHOULD WORK
        
        int random = StdRandom.uniform(size);
        SNode ptr = musicalChairs.getNext();
        SNode prv = musicalChairs;
     ...
     */
    public void insertByHeight(Student studentToInsert) { //CHANGE THIS BACK TO THE ORIGINAL ONE


        SNode eliminatedStudent = new SNode(studentToInsert, null);
        SNode ptr = studentsInLine;
        SNode prv = null;
        if(studentsInLine == null)
        {
           studentsInLine = eliminatedStudent;
        }
        else if(studentToInsert.getHeight() < studentsInLine.getStudent().getHeight())
        {
            eliminatedStudent.setNext(studentsInLine);
           studentsInLine = eliminatedStudent;
        ...
        else
        {
            ...
            prv.setNext(eliminatedStudent);
            eliminatedStudent.setNext(ptr);
        }
    ...
    public void eliminateLosingStudents() { 
        ...
        SNode ptr = musicalChairs.getNext(); //while ptr = head
        while(ptr != musicalChairs) //traverse until ptr reaches last element
        {
                size++;
                ptr = ptr.getNext();
        }
     ...
     */
    public void seatMusicalChairsWinner () {//I THINK IT WORKS

        // WRITE YOUR CODE HERE
        if(musicalChairs == musicalChairs.getNext())//if(musicalChairs.getNext() == musicalChairs) //if the next node is itself
        {
            //insert that student in first empty spot in seatingStudents 2d array
            for(int i = 0; i<studentsSitting.length; i++)
            {
                for(int j = 0; j<studentsSitting[i].length; j++)
                {
                    if(studentsSitting[i][j]==null && seatingLocation[i][j]==false)
                    {
